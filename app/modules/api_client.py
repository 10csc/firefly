...
# ══ 中转客户端（RelayClient）═════════════════════
# 阶段 2.8：实现抽至 modules/api_client_relay.py（纯移动）。
# 以下为 PEP 562 惰性转发：名字首次被访问时才加载实现模块，避免顶层互引成环。
# 打桩语义保持：relay 模块内部对共享件一律经 `api_client.` 模块属性解析，
# 故 `patch("modules.api_client.relay_submit")` / `from modules.api_client import RelayClient`
# / `from modules.api_client import _relay_queues, _relay_lock` 等既有取用方式全部不变。
_RELAY_FORWARD = {
    "RelayClient", "relay_submit", "relay_pending", "relay_has", "relay_result",
    "_relay_queues", "_relay_lock", "_relay_heartbeats",
    "_RELAY_TIMEOUT", "_RELAY_HEARTBEAT_SEC", "_recent_retry_delay",
    "_build_relay_response", "_RelayCompletions", "_RelayChat",
}


def __getattr__(name: str):
    if name in _RELAY_FORWARD:
        from modules import api_client_relay as _relay
        return getattr(_relay, name)
    raise AttributeError(f"module {__name__!r} has no attribute {name!r}")


# ══ Responses API（官方内置 web_search 走这条链，2026-09-15）══════
# 实测：api.deepseek.com/v1/responses 存在且接受 tools:[{type:"web_search"}]；
# chat/completions 无此能力（所以旧链路"模型说自己没有搜索"）。仅 DeepSeek 官方端点启用，
# 其余供应商回退 chat/completions（调用方负责判断）。按 token 计费，无额外搜索费。
def responses_chat(api_key: str, base_url: str, model: str, system: str, user: str,
                   web_search: bool = False, timeout: float = 60.0,
                   max_output_tokens: int = 4000):
    """Responses API 最小调用：instructions + input → 文本。返回兼容文本字符串。
    web_search=True 时带官方联网搜索工具（当前仅 deepseek 官方端点支持）。"""
    ...
    req = _rq.Request(base_url.rstrip("/") + "/responses",
    ...
    try:
        ...
        try:
            ...
            pass
        raise ApiError(f"Responses API 错误 {e.code}: {detail}", code="server_error")
    except Exception as e:
        raise ApiError(f"Responses API 网络错误: {e}", code="network")
    # output: [{type:"message", content:[{type:"output_text", text}]}]（可能含 tool 调用项）
    for item in data.get("output") or []:
        if item.get("type") == "message":
            for c in item.get("content") or []:
                if c.get("type") == "output_text" and c.get("text"):
                    return c["text"]
    raise ApiError("Responses API 返回无文本", code="bad_response")
