...
from modules.app_config import BASE_DIR as _BASE_DIR, USER_DIR as _USER_DIR, user_scope_key
_REGISTRY_FILE = _USER_DIR / "stickers" / "registry.json"
# 内置注册表（app/assets/stickers/registry.json）：用 cfg.BASE_DIR 定位，**不要用 __file__ 相对深度**
# —— 2.7 归位到 domain/stickers/ 后深度变了，`Path(__file__).parent.parent` 会指到 app/domain/assets/
_REGISTRY_LEGACY = _BASE_DIR / "assets" / "stickers" / "registry.json"
...
def migrate_sticker_seed() -> None:
    """一次性播种/迁移（**启动链显式调用**，阶段 2.7 收口）：

    ① `_migrate_legacy_registry()`：把内置 `app/assets/stickers/registry.json` 拷进
       `user_data/stickers/registry.json`（只读公共池 / 本地版用户注册表底座）；
    ② `_migrate_enabled_defaults()`：把新版内置的 enabled/label 合并进已有用户注册表。

    两者都幂等。**原本在模块级直接调用**——于是 `import tools.sticker_picker` 这个纯读动作
    会往真实 user_data 写文件（tests 不得不在 import 之前重定向 USER_DIR 来躲开）。
    现改为由入口启动链显式调用一次（本地版与服务器版各自，见 app/server.py 与 server/app.py）。
    注意：仍然**不要**放进 `_load_registry()`——测试替换 `_REGISTRY_FILE` 后每次加载都迁移
    会把真实数据拷进测试目录。
    """
