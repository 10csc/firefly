...
| [安全加固执行记录-2026-08-25](安全加固执行记录-2026-08-25.md) | 三端安全审查修复：网关 OOM 帽 / relay SSRF 双层拦截 / token 哈希化 / img_id 白名单 / allowBackup（含定性修正与部署提示） |
| [需求设计](需求设计.md) | 需求总览 V5 |
| [消息引用与收藏](消息引用与收藏.md) | 长按消息 → 引用/收藏（数据格式、存储、接口、测试） |
| [部署与发布约定](部署与发布约定.md) | 提交/推送/发布/部署/归档硬规则 |
| [搜索资料结果/](搜索资料结果/) | 调研笔记 |
| [前端/](前端/) | 前端优化与重构方案：PC 端 IM 双栏重构 + 安卓端 UI 优化（2026-08-17 立项，待评审） |
